...
NBS_BANK_URL = 'https://nbs.sk/en/press/news-overview/'
...
class ArticleSpider(scrapy.Spider):
    ...
    def parse_page(self, response):
        ...
        for page in archive_results:
            article_url = page.get_attribute('href')
    ...
    def parse_article(self, response):
        ...
        try:
            ...
            if self.count < self.ARTICLES_MAX_COUNT:
                article_data = self.__clean_article_data(date, article_url, links, labels, body)
    ...
    def create_article(self, article_data: dict):
        ...
        if not check_article_url_existence_in_db(db, item['url']):
            self.__map_to_article_dto(item, db)
    ...
    @staticmethod
    def __map_to_article_dto(item: items.ArticlesItem, db: Session = db):
